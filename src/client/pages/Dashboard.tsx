...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      {err ? (
        <p className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">{err}</p>
      ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-semibold text-slate-900">Recent ingestions</h3>
            <Link to="/team" className="text-xs font-medium text-slate-600 hover:text-slate-900">
              Team view →
            </Link>
          </div>
          <ul className="mt-4 space-y-3">
            {(sync?.recent_ingestions ?? []).map((r, i) => (
              <li key={`${r.note_id}-${i}`} className="border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                <p className="font-medium text-slate-900 line-clamp-2">{r.title || "Untitled"}</p>
            ...
            {!sync?.recent_ingestions?.length ? (
              <li className="text-sm text-slate-500">No recent ingestions yet.</li>
            ) : null}
          </ul>
        </section>

        <section className="rounded-xl border border-amber-200/80 bg-amber-50/60 p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-amber-950">Needs triage</h3>
