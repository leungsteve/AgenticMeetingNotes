...
const ESQL_TOOLS = [
  ...
  // ── SA 1-2-3 Salesforce update tools ─────────────────────────────────────
  {
    ...
    description: "Fetch meetings for an account in the last 7 days. Second leg of the SA 1-2-3 Salesforce update: What did we do this week? (Section 1 is always 'Do we have the tech win?' — call aia.get-sa-tech-win-status for that.) Optionally filter by opportunity name.",
  ...
  },
  {
    ...
    description: "List all open action items for an account. Third leg of the SA 1-2-3 Salesforce update: What are we planning to do next? Returns description, owner, and due date sorted soonest-first.",
  ...
  },
  {
    ...
    description: "Get the most recent meeting notes for an account to assess tech win status. FIRST leg of the SA 1-2-3 Salesforce update: Do we have the tech win and why? — this is always section 1 of the output, because it is Kevin's #1 ask. Evaluate sales_stage, customer_sentiment.overall, decisions_made, open_questions, and tags to determine status.",
...
- **granola-meeting-notes** — Full meeting notes. Key fields: note_id, account (keyword), meeting_date, title, author_name, author_role, author_email, meeting_type, summary, key_topics, decisions_made, open_questions, transcript, decision_process (text — MEDDPICC dimension capturing the customer's internal approval flow: named approvers, gates, sequence, expected timing), technical_environment.current_stack / pain_points / requirements / constraints / scale / integrations, action_items (nested: description, owner, due_date, status), commitments (nested: description, committed_by, timeline), customer_sentiment.overall (keyword: enthusiastic/positive/neutral/cautious/concerned/skeptical), competitive_landscape.competitors_evaluating / incumbent / mentions, budget_timeline.budget / timeline / procurement / stage_signals, tags (keyword[]), sales_stage, attendee_names (keyword[], email addresses)
...
- aia.get-sa-tech-win-status (account)  ← section 1
- aia.get-sa-this-week (account)        ← section 2
- aia.get-sa-open-items (account)       ← section 3
...
Format the output as three clearly labelled sections in this fixed order — never reorder. Each section must be exactly 2-3 sentences. No bullet points. The SA should be able to copy and paste the output directly into Salesforce.

1. **Do we have the tech win?** — direct assertion of RYG, then path_to_tech_win and tech_status_reason. Cite the most recent note (note_id, date) at the end of this section.
2. **What we did this week** — past tense, drawn from this week's summaries and decisions_made.
3. **What we are doing next** — present/future tense, drawn from open action items and the next milestone.

Tech win is always section 1 because it is Kevin's #1 ask and the question the customer's exec sponsor leads with.

## MEDDPICC Qualification

When the user asks about MEDDPICC (or the legacy spelling MEDPICC), use the per-opportunity rollup's medpicc object. There are 8 dimensions, scored out of 8: Metrics, Economic Buyer, Decision Criteria, **Decision Process**, Paper Process, Identify Pain, Champion, Competition. Decision Process is captured on each note's decision_process field and rolls up to medpicc.decision_process. When asked which deals are missing a dimension, prefer ES|QL queries against opportunity-rollups filtering on medpicc.{dimension}.covered == false. Always quote the evidence string for covered dimensions and call out the source_note_id so the user can drill into the original meeting.
...
11. **aia.get-sa-tech-win-status** — SA tech win assessment (1-2-3 update **leg 1** — always section 1)
12. **aia.get-sa-this-week** — SA weekly activity (1-2-3 update leg 2)
13. **aia.get-sa-open-items** — SA open items across all accounts (1-2-3 update leg 3)
...
- For 1-2-3 updates: do NOT ask for an email address. Scope to the named account or opportunity. Tech win is always section 1, never section 3.`;
